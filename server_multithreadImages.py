...
def processImages(initialMsg, addr, sock):
    ...
    while not '}' in list(rcvstr):
        data, address = imgSock.recvfrom(4096)
        #print(data.decode())
    ...
    # display
    log("["+datetime.datetime.now().strftime("%H:%M:%S")+"] [Img Processor] Image '"+ij["name"]+"':", l_file)
    ...
    log("["+datetime.datetime.now().strftime("%H:%M:%S")+"] [Img Processor] Sending image to usrs", l_file)
    for i in range(0,10):
        sendspl.append("")
    for o in usr:
        imgSock.sendto(bytes('!img '+sendspl[0], 'utf-8'),(o,4243))
        time.sleep(0.001)
        ...
        for i in range(0,int((a)/10)+1):
            ...
            try:
                ...
                try:
                    ...
                    pass
                imgSock.sendto(bytes(x,'utf-8'),(o,4243))
            except Exception as e:
                #print(e)
                pass
            time.sleep(0.01)
    log("["+datetime.datetime.now().strftime("%H:%M:%S")+"] [Img Processor] Done Sending", l_file)
    imgSock.close()
